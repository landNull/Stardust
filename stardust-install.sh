...
write_etc_conf() {
  ...
  if [ -f "$dest" ]; then
    echo "global config exists: $dest (not overwritten)"
    return 0
  fi
...
# Stardust System Global Configuration — generated by $PROG
STARDUST_ROLE=$ROLE
...
maybe_gitea_defaults() {
  ...
  host=$(install_prompt "Git SSH host — name in git@HOST:org/repo.git" "$host_def"     "Accept the scanned default. This is usually a Host line in ~/.ssh/config. Empty host skips git template. Type ? here for this text again.")
  owner=$(install_prompt "Git owner/org — first path after the colon" ""     "On Gitea this is the organization or your username. Template becomes git@HOST:OWNER/%s.git (%s = platform name).")
  if [ -z "$owner" ]; then
...
ship_tools() {
  ...
  if [ -x "$TUIDIR/stardust-tui" ]; then
    ...
    (cd "$TUIDIR" && GOPROXY=https://proxy.golang.org,direct go build -o stardust-tui .) &&       install_tool "$TUIDIR/stardust-tui" /usr/local/bin/stardust-tui 0755
  else
